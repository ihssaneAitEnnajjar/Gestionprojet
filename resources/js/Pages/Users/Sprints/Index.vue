...
<template>
    <div>
      ...
      <!-- New Project button -->
      <div class="flex justify-between">
          <a href="/users/sprints/create" class="px-3 py-2 text-white font-semibold bg-oceaned hover:bg-indigo-700 rounded flex">
    ...
    <div>
        ...
        <div class="max-w-7xl mx-auto py-4">
          ...
          <div class="mt-6">
            <Table >
              <template #header>
                <TableRow class="text-sm  border-gray-100	">
              ...
              <template #default>
  <TableRow v-for="sprint in managedSprints" :key="sprint.id" class="border-b">
    <TableDataCell>{{ sprint.name }}</TableDataCell>
    <TableDataCell>{{ sprint.due_date }}</TableDataCell>
    <TableDataCell>
      <input type="text" class="status-input flex items-center" v-model="sprint.status" readonly :class="{'bg-yellow-400 text-black':sprint.status=='Dev In progress','bg-green-500 text-black':sprint.status=='Done','bg-indigo-400 text-black':sprint.status=='To do','bg-red-500 text-black':sprint.status=='Dev Blocked'}"/>
    </TableDataCell>
    <TableDataCell class="priority-cell">
  <span class="priority-label btn-primary flex items-center" :class="{'bg-red-500 text-black':sprint.priority=='HIGH','bg-orange-400 text-black':sprint.priority=='LOW','bg-oceaned text-black':sprint.priority=='MEDIUM'}">{{ sprint.priority  }} </span>
</TableDataCell>
    <TableDataCell class="space-x-4 action-column">
      <div class="action-wrapper">
        <Link :href="route('users.sprints.edit', { id: sprint.id })" class="text-black-400 hover:text-black mr-4">Edit</Link>
...
watch(search, (value) => {
  ...
  if (sprintId.value) {
    router.delete(route('users.sprints.destroy', { id: sprintId.value }));
