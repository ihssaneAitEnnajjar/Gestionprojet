...
watch(search, (value) => {
  ...
  Inertia.get(
    route("admin.permissions.index"), // Make sure the route name is correct
    { search: value },
    {
      preserveState: true,
      replace: true,
...
<template>
  ...
  <AdminLayout>
    <div class="flex justify-between">
      ...
      <div class="flex justify-between">
          <a href="/admin/permissions/create" class="px-3 py-2 text-white font-semibold bg-oceaned hover:bg-indigo-700 rounded flex">
          ...
          <template #default>
            <TableRow v-for="permission in permissions" :key="permission.id" class="border-b">
              ...
              <TableDataCell class=" space-x-4 action-column flex justify-center items-center">
                <Link :href="route('admin.permissions.edi',{id:permission.id} )"  class="text-black hover:text-green-600">Edit</Link>
                <Link :href="route('admin.permissions.destroy',{id:permission.id} )"  class="text-red-400 hover:text-green-600">DElete</Link>
